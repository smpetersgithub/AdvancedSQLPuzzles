...
The function defined in the script is called dbo.fnCompareStrings. The function takes two input strings, @string1 and @string2, and returns a BIT value indicating whether at least one letter (case-sensitive) from @string1 is found in @string2. The function ignores non-letter characters, such as commas and hyphens. Several examples are provided to illustrate how to use the function.
...
The fifth table, #OrdersSetsOverEvaluationFinal, is used to generate all possible combinations of line item pairs from #OrdersSetsOverEvaluation.
