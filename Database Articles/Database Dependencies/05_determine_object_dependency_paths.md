...
Next, we’ll focus on determining dependency paths. 
...
For example, here is a dependency path for the stored procedure, `WideWorldImporters.Website.SearchForPeople`.  Given the output, we can determine all objects referenced by the stored procedure.
...
When calling stored procedures, note the below usage where the three-part naming convention is used for the object name, and a comma-separated list can be used to pass multiple databases.
...
🔍  The following example demonstrates how to trace dependency paths within a single database where the object resides.
...
🔍  The following example demonstrates how to trace dependency paths across multiple databases.
...
Use one of the following stored procedures based on the direction of analysis. You’ll need to pass the target object using a three-part naming convention as a parameter:
