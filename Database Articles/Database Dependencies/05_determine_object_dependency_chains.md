...
By leveraging the `sys.sql_expression_dependencies` table, this script generates dependency chains to trace object relationships, determine their depth, and identify root nodes.
...
The examples reference Microsoft’s `WorldWideImporters` database, which is publicly available online.

For example, here is a dependency chain for the stored procedure, `WideWorldImporters.Website.SearchForPeople`.
...
2. Loop through each object, calculating its dependency chain individually.
